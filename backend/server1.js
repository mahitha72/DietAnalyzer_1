...
// Fetch last 7 charts for weekly report
app.get("/api/charts/weekly-report", async (req, res) => {
  const { email } = req.query;
  try {
    const charts = await Chart.find({ email }).sort({ _id: -1 }).limit(7);
    res.status(200).json(charts);
  } catch (err) {
    res
      .status(500)
      .json({ message: "Error fetching weekly report", error: err.message });
  }
});

// Fetch last 30 charts for monthly report
app.get("/api/charts/monthly-report", async (req, res) => {
  const { email } = req.query;
  try {
    const charts = await Chart.find({ email }).sort({ _id: -1 }).limit(30);
    res.status(200).json(charts);
  } catch (err) {
    res
      .status(500)
      .json({ message: "Error fetching monthly report", error: err.message });
  }
});
